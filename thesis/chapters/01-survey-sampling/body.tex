...
\caption{Types of Weights}
        ...
        \centering
        \vspace*{0.2cm}
\includegraphics[width=0.6\textwidth]{chapters/01-survey-sampling/figures/weighting.png}
\end{figure}
...
where $\sigma_h^2$ denotes the variance of the population parameter of interest $Y$ within stratum $h$, and $n_h$ is the sample size in stratum $h$, such that $\sum_h n_h = n$. When the outcome is observed during recruitment, $\sigma_h^2$ can be estimated directly (e.g., by the sample variance $s_h^2$). Given a fixed total sample size $n$, this variance is minimized by sampling each stratum in proportion to both its weight and its outcome dispersion, $n_h \propto W_h \sigma_h$, the classical Neyman allocation (\citealt{Neyman1934,Groves2010}). In an ideal scenario without any barriers to recruitment, the researcher could implement this allocation directly. In practice, however, recruiting individuals from different strata involves friction, and this friction can vary significantly across strata, making direct application of the Neyman allocation challenging.
...
\[
\begin{aligned}
\argmin_{n_1, \ldots, n_H} &\quad \sum_{h} \frac{W_h^2 \sigma_h^2}{n_h} \\
...
Each term $W_h^2\sigma_h^2/n_h$ is convex in $n_h$ and the constraints are linear, so the program is convex and its first-order (KKT) conditions are sufficient for a global optimum. When the budget constraint binds, the solution has a simple closed form. Forming the Lagrangian
$$
\mathcal{L} = \sum_h \frac{W_h^2\sigma_h^2}{n_h} + \lambda\Big(\sum_h p_h n_h - B\Big),
$$
setting $\partial\mathcal{L}/\partial n_h = 0$ gives $n_h = W_h\sigma_h/\sqrt{\lambda p_h}$, and substituting into the budget constraint to eliminate $\lambda$ yields the cost-aware optimal allocation
$$
n_h^\star = \frac{W_h\sigma_h/\sqrt{p_h}}{\sum_k W_k\sigma_k\sqrt{p_k}}\,B.
$$
Budget thus shifts toward strata that are higher-variance and away from those more expensive per respondent. When the sample cap $N_d$ binds instead of the budget, the same Lagrangian applies with the count constraint $\sum_h n_h = N_d$ in place of $\sum_h p_h n_h = B$; the price no longer appears, and the first-order conditions give $n_h^\star\propto W_h\sigma_h$---headcount split in proportion to weight and outcome dispersion.
...
When the outcome is observed during recruitment, the per-stratum variance $\sigma_h^2$ can be estimated each round in the same spirit as the price. Placing a conjugate prior on the outcome within each stratum---Beta--Binomial for a binary outcome, with Dirichlet and Normal--Inverse-Gamma analogues for ordinal and continuous outcomes---yields a closed-form posterior whose mean we use as $\hat\sigma_h^2$. A weakly informative (Jeffreys) prior keeps every $\hat\sigma_h^2$ strictly positive, so that no stratum is locked out of the allocation before it has been sampled.
...
\begin{algorithm}
\caption{Optimizing Stratified Recruitment with Unknown Costs and Variances}
\begin{algorithmic}
...
\For{$t \in T$} \Comment{\footnotesize Iterate over time steps}
  ...
  \State $\hat\sigma^2_t := []$ \Comment{\footnotesize Initialize variance estimates for $H$ strata}
  \For{$h \in H$} \Comment{\footnotesize Iterate over all strata}
    ...
    \State $\hat\sigma^2_{ht}$ := EstimateVariance$(h)$ \Comment{\footnotesize Posterior-mean variance from responses in stratum $h$}
  \EndFor
  \State $B_{t+1}$ := Optimize$(W, B, N_d, n, p_t, \hat\sigma^2_t)$ \Comment{\footnotesize Optimize budget for the next time step}
...
A useful simplification arises when the outcome is not observed during recruitment---for instance, when it is measured only in a later survey wave---so that $\sigma_h^2$ cannot be estimated from incoming responses, or when the researcher simply wishes to fix the allocation to demographic targets in advance. In these cases it is common to assume equal variance across strata, $\sigma_h^2 = \sigma^2$, under which the design variance reduces to $\mathbb{V}[\bar{y}] = \sigma^2 \sum_h W_h^2/n_h$ and the optimal allocation no longer depends on the outcome: when the budget binds it reduces to $n_h^\star \propto W_h/\sqrt{p_h}$, and absent cost differences to the proportional allocation $n_h^\star \propto W_h$. In this case the \textsc{EstimateVariance} step is skipped and $\hat\sigma^2_h$ is held fixed at a constant; the rest of the algorithm above is unchanged. This is the assumption we adopt in our empirical application, where the recruitment optimization targets demographic representativeness directly.
...
\caption{Distribution of Stratification Variables and Targets from Census}
        \label{fig:strata}
        \centering
\hspace*{-1.2cm}\includegraphics[width=1.1\textwidth]{chapters/01-survey-sampling/figures/strata.pdf}
\end{figure}
...
\caption{Distribution of Socio-demographic Variables (not used in stratification)}
        ...
        \centering
\hspace*{-1.2cm}\includegraphics[width=1.1\textwidth]{chapters/01-survey-sampling/figures/demographics.pdf}
\end{figure}
...
\caption{MAD for Strata, Socio-demographics, and Acquisition Costs Over Time}
        ...
        \centering
        \vspace*{0.2cm}
\includegraphics[width=1\textwidth]{chapters/01-survey-sampling/figures/mad_linear_acquisition_costs_strata and demo.png}
\end{figure}
...
\caption{MAD for Outcome Domains Over Time}
        ...
        \centering
        \vspace*{0.1cm}
\includegraphics[width=1\textwidth]{chapters/01-survey-sampling/figures/MAD_categories.pdf}
\end{figure}
...
\caption{MAD Comparison: Meta, Prolific, and LLM-generated Samples}
        ...
        \centering
        \vspace*{0.1cm}
\includegraphics[width=0.8\textwidth]{chapters/01-survey-sampling/figures/mad_grouped_meta_prolific_llm.pdf}
\end{figure}
...
\caption{MAD Comparison: Meta, Prolific, and LLM-generated Samples}
        ...
        \centering
        \vspace*{0.1cm}
\includegraphics[width=1\textwidth]{chapters/01-survey-sampling/figures/mad_grouped_meta_prolific_llm_bydomain.pdf}
\end{figure}
...
\centering
 \vspace*{0.1cm}
\label{fig:dashboard}
\includegraphics[width=8cm]{chapters/01-survey-sampling/figures/vlab-dash-1b.png} 
\includegraphics[width=8cm]{chapters/01-survey-sampling/figures/vlab-dash-2b.png}
\end{figure}
...
\label{fig:creatives}
\centering
\includegraphics[width=11cm]{chapters/01-survey-sampling/figures/US-ad1.png}
\includegraphics[width=11cm]{chapters/01-survey-sampling/figures/US-ad2.png}
\end{figure}
...
\begin{itemize}[itemsep=-10pt,topsep=0pt]
\item United States
\item Outside the US
\end{itemize}
...
\caption{Overall MAD for Outcome Variables Over Time}
        ...
        \centering
        \vspace*{0.1cm}
\includegraphics[width=0.75\textwidth]{chapters/01-survey-sampling/figures/MAD_weighted_unweighted.pdf}
\end{figure}
...
\caption{Socio-economic Status}
        ...
        \centering
        %\vspace*{-0.1cm}
\includegraphics[width=.85\textwidth]{chapters/01-survey-sampling/figures/cps.pdf}
\end{figure}
...
\caption{Financial Satisfaction and Job Security}
        ...
        \centering
        %\vspace*{-0.1cm}
\includegraphics[width=.85\textwidth]{chapters/01-survey-sampling/figures/gss_satisfaction_2024.pdf}
\end{figure}
...
\caption{Perceptions on Health and Services}
        ...
        \centering
        %\vspace*{-0.1cm}
\includegraphics[width=.85\textwidth]{chapters/01-survey-sampling/figures/gss_health_2024.pdf}
\end{figure}
...
\caption{Attitudes Towards Social Issues}
        ...
        \centering
        %\vspace*{-0.1cm}
\includegraphics[width=.85\textwidth]{chapters/01-survey-sampling/figures/gss_attitudes_2024.pdf}
\end{figure}
...
\caption{Trust in Others, Businesses and the Press}
        ...
        \centering
        %\vspace*{-0.1cm}
\includegraphics[width=.85\textwidth]{chapters/01-survey-sampling/figures/gss_trust_2024.pdf}
\end{figure}
...
\caption{Internet Use}
        ...
        \centering
        %\vspace*{-0.1cm}
\includegraphics[width=.85\textwidth]{chapters/01-survey-sampling/figures/gss_internet.pdf}
\end{figure}

\begin{figure}[H]
\caption{Internet Use for Health Information}
        \label{fig:internet-medical}
        \centering
        %\vspace*{-0.1cm}
\includegraphics[width=.85\textwidth]{chapters/01-survey-sampling/figures/gss_internet_medical.pdf}
\end{figure}
...
\caption{Privacy Concerns}
        ...
        \centering
        %\vspace*{-0.1cm}
\includegraphics[width=.85\textwidth]{chapters/01-survey-sampling/figures/gss_privacy.pdf}
\end{figure}
...
\caption{Past Voting Behavior}
        ...
        \centering
        %\vspace*{-0.1cm}
\includegraphics[width=.85\textwidth]{chapters/01-survey-sampling/figures/gss_voting_2024.pdf}
\end{figure}
